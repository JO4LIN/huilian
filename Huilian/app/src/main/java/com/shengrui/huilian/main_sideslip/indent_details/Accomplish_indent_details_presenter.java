 ...
 */
public class Accomplish_indent_details_presenter extends BasePresenter<Accomplish_indent_details_model> {
    ...
    //不能被实例化
    Accomplish_indent_details_presenter() {
    }

    //单例
    private static Accomplish_indent_details_presenter instance;

    public static Accomplish_indent_details_presenter getInstance() {
        if (instance == null) {
            instance = new Accomplish_indent_details_presenter();
        }
        return instance;
    }
    ...
    public void onInit() {
        //捆绑数据
        model = Accomplish_indent_details_model.class;
        //捆绑adapter
        //data是父类的网络请求后的list数据
        adapter = new Accomplish_indent_details_adapter(data);
    }
...
//                break;
    ...
    protected void getData(String url, RequestParams params, FinishCallBack callBack,int agency) {
        ...
        client.getDataClient(url,
                ...
                model,
                callBack,
                new Client.DataHttpResponseHandler<Accomplish_indent_details_model>() {
                    /**
                     * @param status  状态码
                     * @param totalCount  总数
                     ...
                     */
                    @Override
                    public void onSuccess(int status, int totalCount, List<Accomplish_indent_details_model> modelData) {
                        if (status == Config.STATUS_SUCCESS) {
                            //数据压进去list
                            data.addAll(modelData);
                            Accomplish_indent_details_presenter.this.totalCount = totalCount;
                            page++;
                            //发送空消息，状态码赋值到message.what,并刷新
                            Accomplish_indent_details_presenter.this.sendEmptyMessage(status);

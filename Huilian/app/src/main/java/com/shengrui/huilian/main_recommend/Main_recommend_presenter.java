 ...
 */
public class Main_recommend_presenter extends BasePresenter<Main_recommend_model> {
    ...
    //不能被实例化
    Main_recommend_presenter() {
    }

    //单例
    private static Main_recommend_presenter instance;

    public static Main_recommend_presenter getInstance() {
        if (instance == null) {
            instance = new Main_recommend_presenter();
        }
        return instance;
    }
    ...
    public void onInit() {
        //捆绑数据
        model = Main_recommend_model.class;
        //捆绑adapter
        //data是父类的网络请求后的list数据
        adapter = new Main_recommend_adapter(data);
    }
...
//                break;
    ...
    protected void getData(String url, RequestParams params, FinishCallBack callBack) {
        ...
        client.getDataClient(url,
                ...
                model,
                callBack,
                new Client.DataHttpResponseHandler<Main_recommend_model>() {
                    /**
                     * @param status  状态码
                     * @param totalCount  总数
                     ...
                     */
                    @Override
                    public void onSuccess(int status, int totalCount, List<Main_recommend_model> modelData) {
                        if (status == Config.STATUS_SUCCESS) {
                            //数据压进去list
                            data.addAll(modelData);
                            Main_recommend_presenter.this.totalCount = totalCount;
                            page++;
                            //发送空消息，状态码赋值到message.what,并刷新
                            Main_recommend_presenter.this.sendEmptyMessage(status);

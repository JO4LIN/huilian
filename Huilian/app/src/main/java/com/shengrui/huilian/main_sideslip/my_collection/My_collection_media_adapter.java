 ...
 */
public class My_collection_media_adapter extends BaseDataAdapter<My_collection_media_model> {

    private LayoutInflater mInflater;
    public My_collection_media_adapter(List<My_collection_media_model> datas) {
        super(datas);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
       ...
       // 配置图片加载及显示选项（还有一些其他的配置，查阅doc文档吧）
        ...
        if (convertView == null) {
            ...
            holder.media_id=((TextView) convertView.findViewById(R.id.my_collection_id));
       ...
       // holder.media_id.setText(String.valueOf(datas.get(position).getMediaId()));
        holder.my_collection_name.setText(datas.get(position).getMediaName());
        holder.media_id.setText(String.valueOf(datas.get(position).getMediaId()));
        //ImageLoader.getInstance().displayImage( ima[position], holder.media_pic);
        ImageLoader.getInstance().displayImage( datas.get(position).getWechatHead(), holder.my_collection_head,options);
        return convertView;
    }

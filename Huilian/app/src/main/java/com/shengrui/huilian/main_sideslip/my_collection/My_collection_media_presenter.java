 ...
 */
public class My_collection_media_presenter extends BasePresenter<My_collection_media_model> {
    ...
    //不能被实例化
    My_collection_media_presenter() {
    }

    //单例
    private static My_collection_media_presenter instance;

    public static My_collection_media_presenter getInstance() {
        if (instance == null) {
            instance = new My_collection_media_presenter();
        }
        return instance;
    }
    ...
    public void onInit() {
        //捆绑数据
        model = My_collection_media_model.class;
        //捆绑adapter
        //data是父类的网络请求后的list数据
        adapter = new My_collection_media_adapter(data);
    }
...
//                break;
    ...
    protected void getData(String url, RequestParams params, FinishCallBack callBack,int agency) {
        ...
        client.getDataClient(url,
                ...
                model,
                callBack,
                new Client.DataHttpResponseHandler<My_collection_media_model>() {
                    /**
                     * @param status  状态码
                     * @param totalCount  总数
                     ...
                     */
                    @Override
                    public void onSuccess(int status, int totalCount, List<My_collection_media_model> modelData) {
                        if (status == Config.STATUS_SUCCESS) {
                            //数据压进去list
                            data.addAll(modelData);
                            My_collection_media_presenter.this.totalCount = totalCount;
                            page++;
                            //发送空消息，状态码赋值到message.what,并刷新
                            My_collection_media_presenter.this.sendEmptyMessage(status);

 ...
 */
public class Region_slide_choose_presenter extends BasePresenter<Region_slide_choose_model> {
    ...
    //不能被实例化
    Region_slide_choose_presenter() {
    }

    //单例
    private static Region_slide_choose_presenter instance;

    public static Region_slide_choose_presenter getInstance() {
        if (instance == null) {
            instance = new Region_slide_choose_presenter();
        }
        return instance;
    }
    ...
    public void onInit() {
        //捆绑数据
        model = Region_slide_choose_model.class;
        //捆绑adapter
        //data是父类的网络请求后的list数据
        adapter = new Region_slide_choose_adapter(data);
    }
...
//                break;
    ...
    protected void getData( RequestParams params, FinishCallBack callBack,int agency) {
        if (agency==1){
            page = 1;
            data.clear();
        }
        ...
        client.getDataClient(Config.ScreenCityListURL,
                params,
                model,
                callBack,
                new Client.DataHttpResponseHandler<Region_slide_choose_model>() {
                    /**
                     * @param status  状态码
                     * @param totalCount  总数
                     ...
                     */
                    @Override
                    public void onSuccess(int status, int totalCount, List<Region_slide_choose_model> modelData) {
                        if (status == Config.STATUS_SUCCESS) {
                            //数据压进去list
                            data.addAll(modelData);
                            Region_slide_choose_presenter.this.totalCount = totalCount;
                            page++;
                            //发送空消息，状态码赋值到message.what,并刷新
                            Region_slide_choose_presenter.this.sendEmptyMessage(status);
